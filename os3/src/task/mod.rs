...
struct TaskManagerInner {
    ...
    current_task: usize,
    /// syscall counter
    syscall_counter: [Vec<u32>; MAX_APP_NUM],
}
...
lazy_static! {
    ...
    pub static ref TASK_MANAGER: TaskManager = {
        ...
        let mut counter:[Vec<u32>; MAX_APP_NUM]=Default::default();
...
impl TaskManager {
    ...
    fn get_syscall_record(&self) -> Vec<u32> {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let result = inner.syscall_counter[current].clone();
        drop(inner);
        result
    }

    fn get_time_interval(&self) -> usize {
        let mut now = get_time_us()/1_000;
